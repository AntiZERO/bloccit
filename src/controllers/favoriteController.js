 ...
 // `create` checks if there is a user signed in and if so, calls the
 // `createFavorite` method of `queries.favorites.js` with the reqeust.
 // If `createFavorite` returns an error, we load a flash message.
 // Otherwise, the user must not be signed in and we present an error
 // saying so. Finally, we redirect to the view where the request came from.
   create(req, res, next){
     if(req.user){
 ...
 // `destroy` checks if there is a user signed ina nd if so, calls the
 // `deleteFavorite` method of the `queries.favorites.js` with the request.
